...
import type { WorkPacer } from "#rf/utils/scheduler"
import { createWorkPacer, pauseIfBudgetSpent } from "#rf/utils/scheduler"
...
export async function translateWalkedElement(
  ...
  if (element.hasAttribute(PARAGRAPH_ATTRIBUTE)) {
    ...
    // Gmail/atomic mode: never split a labeled paragraph into mid-sentence
    // units when nested Word/highlight blocks carry BLOCK_ATTRIBUTE.
    if (siteRule.keepParagraphAtomic) {
      promises.push(
        translateNodes(
          [element],
          walkId,
          toggle,
          config,
          siteRule.forceBlockBilingual === true,
          forceRetranslation,
        ),
      )
    } else {
